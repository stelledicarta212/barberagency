...
function normalizeMethod(value: unknown): PagoMetodo {
  return value === "digital" ? "digital" : "efectivo";
}

export function PagosCrud() {
  ...
  const [editingRow, setEditingRow] = useState<PagoRow | null>(null);
  const [form, setForm] = useState<PagoForm>(EMPTY_FORM);
  ...
  async function loadRows() {
    ...
    setError(false);
    try {
      const response = await fetch("/api/pagos", { cache: "no-store" });
      const data = (await response.json().catch(() => ({}))) as ApiResponse;

      if (!response.ok || !data.ok) {
        throw new Error(data.message || "No se pudo cargar pagos.");
      }

      setRows(Array.isArray(data.rows) ? data.rows : []);
    } catch (err) {
      const text = err instanceof Error ? err.message : "No se pudo cargar pagos.";
      setMessage(text);
      setError(true);
    } finally {
      setLoading(false);
  ...
  function openEditModal(row: PagoRow) {
    setEditingRow(row);
    setForm({
      cita_id: row.cita_id,
      total: row.total,
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const endpoint = isEdit ? `/api/pagos/${editingRow!.id}` : "/api/pagos";
      const method = isEdit ? "PATCH" : "POST";

      const payload = {
        cita_id: Number(form.cita_id),
        total: Number(form.total),
        metodo: normalizeMethod(form.metodo),
        pagado_en: form.pagado_en ? form.pagado_en : null,
      };
      ...
      setForm(EMPTY_FORM);
      await loadRows();
      setMessage(isEdit ? "Pago actualizado correctamente." : "Pago creado correctamente.");
      setError(false);
    } catch (err) {
      const text = err instanceof Error ? err.message : "No se pudo guardar el pago.";
      setMessage(text);
      setError(true);
    } finally {
      setSaving(false);
  ...
  async function handleDelete(row: PagoRow) {
    if (!canManage) return;
    const confirmed = window.confirm(`Eliminar pago de la cita ${row.cita_id}?`);
    if (!confirmed) return;
    ...
    try {
      const response = await fetch(`/api/pagos/${row.id}`, { method: "DELETE" });
      const data = (await response.json().catch(() => ({}))) as {
        ok?: boolean;
        message?: string;
      };
      ...
      const text = err instanceof Error ? err.message : "No se pudo eliminar el pago.";
      setMessage(text);
      setError(true);
    } finally {
      setDeletingId(null);
  ...
  return (
    <section className="space-y-5">
      <div className="animate-rise flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="title-gradient text-2xl font-black tracking-tight sm:text-3xl">Pagos</h1>
          <p className="mt-1 text-sm text-zinc-400">Registro de cobros por cita (BD en vivo)</p>
        </div>
      ...
      <div className="panel animate-rise overflow-hidden" style={{ animationDelay: "90ms" }}>
        {loading ? (
          <div className="p-4 text-sm text-zinc-400">Cargando pagos...</div>
        ) : rows.length === 0 ? (
          <div className="p-4 text-sm text-zinc-400">No hay pagos registrados.</div>
        ) : (
          <>
            <div className="space-y-3 p-3 md:hidden">
              {rows.map((row) => (
                <article
                  key={`pago-mobile-${row.id}`}
                ...
                >
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-semibold text-zinc-200">{row.cita_id}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                        Total
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-semibold capitalize text-zinc-200">{row.metodo}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                        Pagado en
                      </span>
                      <span className="font-semibold text-zinc-200">{toUiDateTime(row.pagado_en)}</span>
                    </div>
                  </div>
              ...
              <table className="min-w-full border-collapse">
                <thead>
                  <tr className="bg-zinc-950 text-left text-xs uppercase tracking-wide text-zinc-200">
                    <th className="px-4 py-3 font-semibold">Cita ID</th>
                    <th className="px-4 py-3 font-semibold">Total</th>
                    <th className="px-4 py-3 font-semibold">Metodo</th>
                    <th className="px-4 py-3 font-semibold">Pagado en</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    ...
                    >
                      <td className="px-4 py-3">{row.cita_id}</td>
                      <td className="px-4 py-3">{formatMoneyCOP(row.total)}</td>
                      <td className="px-4 py-3 capitalize">{row.metodo}</td>
                      <td className="px-4 py-3">{toUiDateTime(row.pagado_en)}</td>
                      {canManage ? (
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-black tracking-tight text-zinc-100">
                  {editingRow ? "Editar pago" : "Nuevo pago"}
                </h2>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submitForm} className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="space-y-1 text-sm">
                  <span className="text-zinc-300">Cita ID</span>
                  <input
                    required
                    type="number"
                    min={1}
                    value={form.cita_id || ""}
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, cita_id: Number(event.target.value || 0) }))
                    }
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="space-y-1 text-sm">
                  <span className="text-zinc-300">Metodo</span>
                  <select
                    value={form.metodo}
                    onChange={(event) =>
                      setForm((prev) => ({
                        ...prev,
                        metodo: normalizeMethod(event.target.value),
                      }))
                    }
                    className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--surface-muted)] px-3 text-sm text-zinc-100 outline-none focus:border-zinc-500"
                  >
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
                ...
                >
                  ...
                  {saving ? "Guardando..." : editingRow ? "Guardar cambios" : "Crear pago"}
                </button>
              </div>
            </form>
          </article>
