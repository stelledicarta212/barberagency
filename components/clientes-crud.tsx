...
export function ClientesCrud() {
  ...
  async function loadRows() {
    ...
    try {
      ...
      const text = err instanceof Error ? err.message : "No se pudo cargar clientes.";
      setMessage(text);
      setError(true);
    } finally {
      setLoading(false);
  ...
  function openEditModal(row: ClienteRow) {
    setEditingRow(row);
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const endpoint = isEdit ? `/api/clientes/${editingRow!.id}` : "/api/clientes";
      ...
      setForm(EMPTY_FORM);
      await loadRows();
      setMessage(isEdit ? "Cliente actualizado correctamente." : "Cliente creado correctamente.");
      setError(false);
    } catch (err) {
      const text = err instanceof Error ? err.message : "No se pudo guardar el cliente.";
      setMessage(text);
      setError(true);
    } finally {
      setSaving(false);
  ...
  async function handleDelete(row: ClienteRow) {
    if (!canManage) return;
    const confirmed = window.confirm(`Eliminar cliente ${row.nombre}?`);
    if (!confirmed) return;
    ...
    try {
      const response = await fetch(`/api/clientes/${row.id}`, { method: "DELETE" });
      const data = (await response.json().catch(() => ({}))) as {
        ok?: boolean;
        message?: string;
      };
      ...
      const text = err instanceof Error ? err.message : "No se pudo eliminar el cliente.";
      setMessage(text);
      setError(true);
    } finally {
      setDeletingId(null);
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="panel animate-rise overflow-hidden" style={{ animationDelay: "90ms" }}>
        {loading ? (
          <div className="p-4 text-sm text-zinc-400">Cargando clientes...</div>
        ) : rows.length === 0 ? (
          <div className="p-4 text-sm text-zinc-400">No hay clientes registrados.</div>
        ) : (
          <>
            <div className="space-y-3 p-3 md:hidden">
              {rows.map((row) => (
                <article
                  key={`cliente-mobile-${row.id}`}
                ...
                >
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-semibold text-zinc-200">{row.telefono}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                        Visitas
                      </span>
                      <span className="font-semibold text-zinc-200">{row.visitas}</span>
                    </div>
                  </div>
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-400">
                  Registra informacion de contacto del cliente.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submitForm} className="space-y-3">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
                ...
                >
                  ...
                  {saving ? "Guardando..." : editingRow ? "Guardar cambios" : "Crear cliente"}
                </button>
              </div>
            </form>
          </article>
