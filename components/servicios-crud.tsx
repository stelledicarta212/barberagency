...
export function ServiciosCrud() {
  ...
  const [editingRow, setEditingRow] = useState<ServicioRow | null>(null);
  ...
  async function loadRows() {
    ...
    try {
      ...
      const text = err instanceof Error ? err.message : "No se pudo cargar servicios.";
      setMessage(text);
      setError(true);
    } finally {
      setLoading(false);
  ...
  function openEditModal(row: ServicioRow) {
    setEditingRow(row);
    setForm({
      nombre: row.nombre,
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const endpoint = isEdit ? `/api/servicios/${editingRow!.id}` : "/api/servicios";
      ...
      setForm(EMPTY_FORM);
      await loadRows();
      setMessage(isEdit ? "Servicio actualizado correctamente." : "Servicio creado correctamente.");
      setError(false);
    } catch (err) {
      const text = err instanceof Error ? err.message : "No se pudo guardar el servicio.";
      setMessage(text);
      setError(true);
    } finally {
      setSaving(false);
  ...
  async function handleDelete(row: ServicioRow) {
    if (!canManage) return;
    const confirmed = window.confirm(`Eliminar servicio ${row.nombre}?`);
    if (!confirmed) return;
    ...
    try {
      const response = await fetch(`/api/servicios/${row.id}`, { method: "DELETE" });
      const data = (await response.json().catch(() => ({}))) as {
        ok?: boolean;
        message?: string;
      };
      ...
      const text = err instanceof Error ? err.message : "No se pudo eliminar el servicio.";
      setMessage(text);
      setError(true);
    } finally {
      setDeletingId(null);
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="panel animate-rise overflow-hidden" style={{ animationDelay: "90ms" }}>
        {loading ? (
          <div className="p-4 text-sm text-zinc-400">Cargando servicios...</div>
        ) : rows.length === 0 ? (
          <div className="p-4 text-sm text-zinc-400">No hay servicios registrados.</div>
        ) : (
          <>
            <div className="space-y-3 p-3 md:hidden">
              {rows.map((row) => (
                <article
                  key={`servicio-mobile-${row.id}`}
                ...
                >
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-semibold text-zinc-200">{row.nombre}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                        Duracion
                      </span>
                      <span className="font-semibold text-zinc-200">{row.duracion_min} min</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                        Precio
                      </span>
                      <span className="font-semibold text-zinc-200">{formatMoneyCOP(row.precio)}</span>
                    </div>
                  </div>
              ...
              <table className="min-w-full border-collapse">
                <thead>
                  <tr className="bg-zinc-950 text-left text-xs uppercase tracking-wide text-zinc-200">
                    ...
                    <th className="px-4 py-3 font-semibold">Duracion (min)</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">{row.duracion_min}</td>
                      <td className="px-4 py-3">{formatMoneyCOP(row.precio)}</td>
                      {canManage ? (
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-black tracking-tight text-zinc-100">
                  {editingRow ? "Editar servicio" : "Nuevo servicio"}
                </h2>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submitForm} className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="space-y-1 text-sm">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm((prev) => ({
                        ...prev,
                        duracion_min: Number(event.target.value || 0),
                      }))
                    }
                    className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--surface-muted)] px-3 text-sm text-zinc-100 outline-none focus:border-zinc-500"
                  />
                ...
                <label className="space-y-1 text-sm">
                  ...
                  <input
                    ...
                    value={form.precio}
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, precio: Number(event.target.value || 0) }))
                    }
                    className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--surface-muted)] px-3 text-sm text-zinc-100 outline-none focus:border-zinc-500"
                  />
                </label>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
                ...
                >
                  ...
                  {saving ? "Guardando..." : editingRow ? "Guardar cambios" : "Crear servicio"}
                </button>
              </div>
            </form>
          </article>
