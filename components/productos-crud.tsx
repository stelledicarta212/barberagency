...
export function ProductosCrud() {
  ...
  async function loadRows() {
    ...
    try {
      ...
      const text = err instanceof Error ? err.message : "No se pudo cargar productos.";
      setMessage(text);
      setError(true);
    } finally {
      setLoading(false);
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const endpoint = isEdit ? `/api/productos/${editingRow!.id}` : "/api/productos";
      ...
      setForm(EMPTY_FORM);
      await loadRows();
      setMessage(isEdit ? "Producto actualizado correctamente." : "Producto creado correctamente.");
      setError(false);
    } catch (err) {
      const text = err instanceof Error ? err.message : "No se pudo guardar el producto.";
      setMessage(text);
      setError(true);
    } finally {
      setSaving(false);
  ...
  async function handleDelete(row: ProductoRow) {
    if (!canManage) return;
    const confirmed = window.confirm(`Eliminar producto ${row.nombre}?`);
    if (!confirmed) return;
    ...
    try {
      const response = await fetch(`/api/productos/${row.id}`, { method: "DELETE" });
      const data = (await response.json().catch(() => ({}))) as {
        ok?: boolean;
        message?: string;
      };
      ...
      const text = err instanceof Error ? err.message : "No se pudo eliminar el producto.";
      setMessage(text);
      setError(true);
    } finally {
      setDeletingId(null);
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="panel animate-rise overflow-hidden" style={{ animationDelay: "90ms" }}>
        {loading ? (
          <div className="p-4 text-sm text-zinc-400">Cargando productos...</div>
        ) : rows.length === 0 ? (
          <div className="p-4 text-sm text-zinc-400">No hay productos registrados.</div>
        ) : (
          <>
            <div className="space-y-3 p-3 md:hidden">
              {rows.map((row) => (
                <article
                  key={`producto-mobile-${row.id}`}
                ...
                >
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-semibold text-zinc-200">{row.stock}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                        Activo
                      </span>
                      <span className="font-semibold text-zinc-200">{row.activo ? "Si" : "No"}</span>
                    </div>
                  </div>
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-black tracking-tight text-zinc-100">
                  {editingRow ? "Editar producto" : "Nuevo producto"}
                </h2>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submitForm} className="space-y-3">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
                ...
                >
                  ...
                  {saving ? "Guardando..." : editingRow ? "Guardar cambios" : "Crear producto"}
                </button>
              </div>
            </form>
          </article>
