...
async function parsePagoId(paramsPromise: RouteContext["params"]) {
...
async function readPago(id: number, barberiaId: number, token: string) {
  const rows = await postgrestRequest<PagoRow[]>(
    `pagos?select=id,cita_id,total,metodo,pagado_en,citas!inner(barberia_id)&id=eq.${id}&citas.barberia_id=eq.${barberiaId}&limit=1`,
    token,
  );
  return Array.isArray(rows) ? rows[0] : null;
}
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const pagoId = await parsePagoId(context.params);
  if (!pagoId) {
    return NextResponse.json({ ok: false, message: "ID de pago invalido." }, { status: 400 });
  }
  ...
  try {
    const current = await readPago(pagoId, barberiaId, token);
    if (!current) {
      return NextResponse.json(
        { ok: false, message: "El pago no existe o no pertenece a tu barberia." },
        { status: 404 },
      );
    }

    const citaId = toPositiveInt(body.cita_id ?? current.cita_id);
    const total = Math.max(0, toSafeNumber(body.total ?? current.total));
    const metodo = normalizeMetodo(body.metodo) || normalizeMetodo(current.metodo);
    const pagadoEn =
      body.pagado_en === null
        ? null
        : normalizeDateTime(body.pagado_en) || normalizeDateTime(current.pagado_en) || null;
    ...
    const rows = await postgrestRequest<PagoRow[]>(`pagos?id=eq.${pagoId}`, token, {
      method: "PATCH",
      body: {
        cita_id: citaId,
        total,
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    await postgrestRequest<unknown>(`pagos?id=eq.${pagoId}`, token, {
      method: "DELETE",
      preferRepresentation: true,
    });
